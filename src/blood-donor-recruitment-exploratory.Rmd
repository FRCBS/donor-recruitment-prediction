...
p <- p + facet_grid(.~Sex)
p <- p + xlab("Number of consecutive donation event") + ylab("Probability of next donation")
p
```
...
p <- p + facet_grid(.~Sex)
p <- p + scale_y_log10()
p <- p + xlab("Number of consecutive donation event") +  ylab("Delay in days to next donation")
p
```
