...
if (!is.null(load.ready) && load.ready != 1) {
  ...
  if (!is.null(file)) {
    load(file = file)
  } else if (!exists('donationdata') || is.null(donationdata)) {
    stop('Neither path to data file provided in param$data.file nor data available in donationdata (in memory)')
  }
